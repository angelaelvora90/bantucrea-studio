...
```bash
npm start          # sert l'application sur http://localhost:3000
```

Puis ouvrir <http://localhost:3000>. `server.js` est un serveur statique **sans aucune dépendance**
(Node ≥ 18) ; n'importe quel hébergeur statique (GitHub Pages, Netlify, Nginx, S3…) convient aussi :
il suffit de publier le dossier `public/`.
...
`ui.test.js` charge `public/index.html` dans jsdom, exécute réellement `app.js` et vérifie le rendu
SVG, les panneaux, l'historique et les exports.
